...
const createCssLoader = (options = {}, ...loaders) => [
  options.isCssExtract ? {
    ...
    loader: 'css-loader',
    options: { modules: options.isCssModule }
  }
].concat([{
...
const generator = (env = 'development', ic, config) => {
  ...
  return {
    ...
    optimization: (NODE_ENV === 'production') && config.output.library ? {} : {
    ...
    module: {
      rules: [{
        ...
        test: /\.less$/,
        use: createCssLoader(ic, 'less-loader')
      }, {
        test: /\.(scss|sass)$/,
...
module.exports = {
  createCssLoader, generator, createURILoader, createEslintLoader
}
